/**
 * PR.js
 * An HTML5 Canvas Framework designed to resemble the Processing API.
 * 
 * by WANG Yang
 * 
 */
...
(function (global) {
    ...
    global.PR = {
        ...
        launch: function (canvasId, proj) {
            var self = this, interval = 1000 / self.frameRate,
        ...
        /*
         * Start a non-loop app
         */
        run: function (canvasId, proj) {
            var self = this,
                canvas = document.getElementById(canvasId);
            self.context = canvas.getContext("2d");
            self.size(canvas.width, canvas.height);
            ...
            proj.mouseMove && canvas.addEventListener("mousemove", proj.mouseMove);
            proj.mousePressed && canvas.addEventListener("mousedown", proj.mousePressed);
            proj.mouseReleased && canvas.addEventListener("mouseup", proj.mouseReleased);
            proj.keyPressed && canvas.addEventListener("keypress", proj.keyPressed);
    ...
    global.PRProject = (function () {

        function PRProject(w, h) {
            this.width = w;
            this.height = h;
        }
